...
# =====================================================
#  PERMANENT LINK BUILDER — survives bot bans  ⭐
# Don't Remove Credit
# Made by @MrYKTG https://github.com/MrYKTG
# @mrxeontg @xeonbotz
# Ask Doubt on telegram @Mrxeontg
...
# All Rights Reserved.
# =====================================================
def build_permanent_link(payload: str,
                         bot_username: str | None = None,
                         param: str | None = None) -> str:
    """
    Returns a permanent gateway URL like:
        https://<GATEWAY_BASE_URL>/?<PARAM>=<payload>

    The URL is read from config.GATEWAY_BASE_URL — never hardcoded here.
    The website then redirects the user to whichever bot is currently
    listed in its BOT_USERNAMES array. So this URL is bot-ban-proof.
    """
    if PERMANENT_LINKS and GATEWAY_BASE_URL:
        base = GATEWAY_BASE_URL.rstrip("/")
        p = (param or PERMANENT_START_POINT or "start").strip()
        return f"{base}/?{p}={payload}"
...
def generate_links(param: str, bot_username: str):
    ...
    permanent_link = build_permanent_link(param, bot_username=bot_username)

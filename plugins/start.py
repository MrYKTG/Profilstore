...
from plugins.pro_caption import build_pro_caption
...
async def start_command(client: Client, message: Message):
    ...
    if len(text) > 7:
        ...
        if not is_premium_user and token_verification_enabled and not verified_via_token:
                        ...
                        for chan in caption_channels:
                            try:
                                ...
                                if f_msg and not f_msg.empty:
                                    if f_msg.document:
                                        content_name = f_msg.document.file_name
                                    break
                            except:
                                continue
                    except:
            ...
            # ── Determine credit system state & reward ──
            credit_system_enabled = await client.mongodb.is_credit_system_enabled()
            verification_reward = await client.mongodb.get_bot_config('verification_reward', 3)

            # ══════════════════════════════════════════════════
            #  CAPTION FORMAT — based on credit system status
            # ══════════════════════════════════════════════════
            if credit_system_enabled:
                # ── Credit system ON format ──
                caption = (
                    f"<b>🎬 {content_name}</b>\n\n" if content_name else ""
                ) + (
                    "<b>‼️ ʏᴏᴜʀ ʟɪɴᴋ ɪꜱ ʀᴇᴀᴅʏ ‼️</b>\n"
                    "<b>────────────────────</b>\n"
                    "<b>◍ ᴘʟᴇᴀsᴇ ᴠᴇʀɪғʏ ᴛᴏ ʀᴇɢᴀɪɴ ᴀᴄᴄᴇss ᴛᴏ ᴛʜᴇ ғɪʟᴇs</b>\n"
                    f"<b>◍ ᴇᴀᴄʜ ᴀᴅ ʙʏᴘᴀss ʀᴇᴡᴀʀᴅs ʏᴏᴜ ᴡɪᴛʜ {verification_reward} ᴄʀᴇᴅɪᴛs.</b>\n"
                    "<b>◍ ᴏɴᴇ ᴄʀᴇᴅɪᴛ ɪs ᴄᴏɴsᴜᴍᴇᴅ ᴘᴇʀ ғɪʟᴇ/ʟɪɴᴋ ᴀᴄᴄᴇss.</b>"
                )
            else:
                # ── Credit system OFF format ──
                caption = (
                    "<b>✦ ʜᴇʏ ʙʀᴏ/sɪs,</b>\n\n"
                    "<b>‼️ ɢᴇᴛ ᴀʟʟ ꜰɪʟᴇꜱ ɪɴ ᴀ ꜱɪɴɢʟᴇ ʟɪɴᴋ ‼️</b>\n\n"
                    "<b>⌯ ʏᴏᴜʀ ʟɪɴᴋ ɪꜱ ʀᴇᴀᴅʏ, ᴋɪɴᴅʟʏ ᴄʟɪᴄᴋ ᴏɴ ᴏᴘᴇɴ ʟɪɴᴋ ʙᴜᴛᴛᴏɴ..</b>"
                )

            # ══════════════════════════════════════════════════
            #  BUTTONS — cleaned corners, new titles
            # ══════════════════════════════════════════════════
            buttons = InlineKeyboardMarkup([
                [InlineKeyboardButton("◍ ᴏᴘᴇɴ ʟɪɴᴋ ◍", url=masked["masked_url"])],
                [
                    InlineKeyboardButton("◍ ᴛᴜᴛᴏʀɪᴀʟ", url="https://t.me/CulturedxSaga/109"),
                    InlineKeyboardButton("ʙᴜʏ ᴘʀᴇᴍɪᴜᴍ ◍", url="https://t.me/Culturedxsga/100")
                ]
            ])
        ...
        for msg in valid_messages:
            caption = build_pro_caption(msg, client=client)
            try:
                ...
                client.LOGGER(__name__, client.name).warning(
                    f"Failed to copy message {msg.id}: {e}"
                )
        ...
        if yugen_msgs and client.auto_del > 0:
            minutes_text = humanize.naturaldelta(client.auto_del)
            warning = await client.send_message(
                user_id,
                (
                    f"<blockquote><b>⧗ Dᴜᴇ ᴛᴏ Cᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs....</b></blockquote>\n"
                    f"<blockquote><b>›› Yᴏᴜʀ ғɪʟᴇs ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇᴅ ᴡɪᴛʜɪɴ {minutes_text}. Sᴏ ᴘʟᴇᴀsᴇ ғᴏʀᴡᴀʀᴅ ᴛʜᴇᴍ ᴛᴏ ᴀɴʏ ᴏᴛʜᴇʀ ᴘʟᴀᴄᴇ ғᴏʀ ғᴜᴛᴜʀᴇ ᴀᴠᴀɪʟᴀʙɪʟɪᴛʏ..</b></blockquote>\n"
                    f"<blockquote><b>≡ ɴᴏᴛᴇ : ᴜsᴇ <a href=\"https://play.google.com/store/apps/details?id=org.videolan.vlc\">ᴠʟᴄ ᴘʟᴀʏᴇʀ</a> ᴏʀ <a href=\"https://play.google.com/store/apps/details?id=com.mxtech.videoplayer.ad\">ᴍx ᴘʟᴀʏᴇʀ</a> ғᴏʀ ᴀ ʙᴇᴛᴛᴇʀ ᴠɪᴇᴡɪɴɢ ᴇxᴘᴇʀɪᴇɴᴄᴇ !.</b></blockquote>"
                ),
